...
{
    /// <summary>
    /// Metadata that <see cref="WorkerApplicationFactory{TEntryPoint}"/> uses to find out the content
    /// root for the worker application represented by <c>TEntryPoint</c>.
    /// <see cref="WorkerApplicationFactory{TEntryPoint}"/> will iterate over all the instances of
    /// <see cref="WorkerApplicationFactoryContentRootAttribute"/>, filter the instances whose
    /// <see cref="Key"/> is equal to <c>TEntryPoint</c> <see cref="Assembly.FullName"/>,
    /// order them by <see cref="Priority"/> in ascending order.
    /// <see cref="WorkerApplicationFactory{TEntryPoint}"/> will check for the existence of the marker
    ...
    public sealed class WorkerApplicationFactoryContentRootAttribute : Attribute
    {
        /// <summary>
        /// Initializes a new instance of <see cref="WorkerApplicationFactoryContentRootAttribute"/>.
        /// </summary>
        /// <param name="key">
        /// The key of this <see cref="WorkerApplicationFactoryContentRootAttribute"/>. This
        /// key is used by <see cref="WorkerApplicationFactory{TEntryPoint}"/> to determine what of the
        /// <see cref="WorkerApplicationFactoryContentRootAttribute"/> instances on the test assembly should be used
        ...
        /// multiple <see cref="WorkerApplicationFactoryContentRootAttribute"/> instances are applied for the
        ...
        /// </param>
        public WorkerApplicationFactoryContentRootAttribute(
            string key,
        ...
        /// <summary>
        /// Gets a number for determining the probing order when multiple <see cref="WorkerApplicationFactoryContentRootAttribute"/>
